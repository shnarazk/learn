...
```
$ lake exe verso-serve --port 8000 _out/html-multi
```

を使うこと。

::: comment
literate programmingなら

```
rm -fr .lake/build/literete
lake query :literateHtml
```

This is a sample. 日本語はどうなる?
:::
...
- AddCommGroup（加法交換群, 加法群）:
    ...
    - 単位元、逆元
- Module K V（加群）: 環$`K`, アーベル群$`V`に対して
    - $`K • V → V` が以下を満たす:
        - $`R`に関する分配法則、
        - $`V`に関する分配法則と結合法則,
        - $`R`の単位元は $`(•)` の単位元
...
example (a : EuclideanSpace ℝ (Fin 2)) :
    ‖a‖ = ((a 0)^2 + (a 1)^2).sqrt := by
  have zle (x y : ℝ) : 0 ≤ x^2 + y^2 := by
