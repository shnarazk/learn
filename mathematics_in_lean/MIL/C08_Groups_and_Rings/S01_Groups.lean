...
lemma aux_card_eq [Fintype G] (h' : Nat.card G = Nat.card H * Nat.card K) : Nat.card (G ⧸ H) = Nat.card K := by
  sorry
variable [H.Normal] [K.Normal] [Fintype G] (h : Disjoint H K) (h' : Nat.card G = Nat.card H * Nat.card K)
